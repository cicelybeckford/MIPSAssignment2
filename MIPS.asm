...
main: 
	...
	STOP:	    sub $s6, $s6, $t2          #reinitalize the substring's address pointer
				...
				sw $t2, 4($sp)             #store the substring's length in stack 
...
subprogram_2:	
    ...
    CHECK:	    lb $s4, ($a2)				#load character into $s4
    ...
    NOSPACE:    addi $sp, $sp, -4           #reserve space in stack 
				...
				lw $ra, 12($sp)             #move the old return address from stack back into $ra
